              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    Color? iconColor = method.isActive ? AppColors.primaryColor : Colors.grey;
                    ...
                    if (isWalletMethod) {
                      if (isWalletLoading) {
                        ...
                        isMethodUsable = false;
                        subtitleOverride = "Insufficient Balance: ₹${currentWalletBalance.toStringAsFixed(2)}";
                        iconColor = Colors.red;
                      } else {
                        subtitleOverride = "Balance: ₹${currentWalletBalance.toStringAsFixed(2)}";
                        iconColor = AppColors.primaryColor;
                      }
                    }

                    return ListTile(
                      title: Text(method.paymenName, style: GoogleFonts.poppins()),
                      subtitle: Text(
                          subtitleOverride ?? method.discraption,
                          style: GoogleFonts.poppins(
                              fontSize: 10,
                              color: isMethodUsable ? Colors.black54 : Colors.redAccent
                          )
                      ),
                      leading: Icon(
                        isWalletMethod ? Icons.wallet_giftcard : (isMethodUsable ? Icons.payment : Icons.lock_outline),
                        color: iconColor,
                      ),
                      trailing: isMethodUsable && _selectedMethod?.id == method.id
                      ...
                      onTap: isMethodUsable
                        ...
                        Navigator.pop(context);
                          ...
                          : () {
                        if (isWalletMethod && !isWalletLoading && currentWalletBalance < widget.totalAmount) {
                  ...
                  BlocConsumer<PaymentCubit, PaymentState>(
                    ...
                    builder: (context, state) {
                      ...
                      if (state is PaymentLoaded) {
                        ...
                        // The default method is now set in the listener, so we just display it.
                        if (_selectedMethod != null) {
                          paymentText = _selectedMethod!.paymenName;
                        } else {
                          paymentText = 'Select Payment Method'; // Fallback if listener failed/wasn't called yet
                        }
                      ...
                      children: [
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton(
                            // The button is enabled if a method is selected AND the user is logged in (userId != 0)
                            onPressed: _selectedMethod != null && widget.userId != 0
                                ? () {
                              Navigator.of(context).push(
                                SlidePageRoute(
                                  page: OrderReviewScreen(
                                    ...
                                    deliveryAddress: widget.deliveryAddress, selectedAddressId: widget.selectedAddressId,
                                  ),
                                ),
                              );
                            }
                                : widget.userId == 0
                                ? () { // Handle not logged in case to show Snackbar
                              context.read<DashboardBloc>().add(DashboardTabChanged(4));
                              showAppSnackBar(context, "You firstly need to Login to place order");
                            }
                                : null, // Disable if method is null and user is logged in
